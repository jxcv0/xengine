...
public:
  ...
  {
    m_arr.erase(
        std::remove_if(std::execution::par_unseq, m_arr.begin(), m_arr.end(),
  ...
  {
    auto it
        = std::find_if(std::execution::par_unseq, m_arr.cbegin(), m_arr.cend(),
                       [=](const auto& c) { return c.entity == entity; });
    if (it != m_arr.cend())
      {
        return (*it).value;
  ...
  {
    auto it
        = std::find_if(std::execution::par_unseq, m_arr.begin(), m_arr.end(),
                       [=](const auto& c) { return c.entity == entity; });
    if (it != m_arr.end())
      {
        (*it).value = val;
