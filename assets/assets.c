...
struct assetmgr_shm *
init_assetmgr_shm (void)
{
  int fd = shm_open (ASSETMGR_SHMPATH, O_CREAT | O_EXCL | O_RDWR,
                     S_IRWXU | S_IRWXG);
...
int
register_asset (struct assetmgr_shm *shm, const char *filepath)
{
...
char *
asset_path (struct assetmgr_shm *shm, int assetid)
{
  return NULL;
}

int
load_assets (struct assetmgr_shm *shm, int nassets, int *ids)
{
  // TODO
  return -1;
}
...
int
unload_assets (struct assetmgr_shm *shm, int nassets, int *ids)
{
  // TODO
  return -1;
}
...
int
assetmgrd_process_requests (struct assetmgr_shm *shm)
{
